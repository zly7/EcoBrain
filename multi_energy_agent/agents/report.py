...
- 提供可操作的建议和下一步行动计划"""
        ...
        # Build user prompt - 按照绿色园区政策文件结构
        user_prompt = f"""请基于以下数据生成一份专业的绿色园区低碳发展规划报告。
...
请按照中国绿色园区政策文件的标准结构，生成一份专业的规划报告（总字数不少于 4000 字）：

# 报告结构要求（参照国家绿色园区政策文件格式）

## 执行摘要
（300-500字，概述园区基本情况、核心发现、主要建议和预期效益）
...
**报告要求**：
1. 严格按照上述政策文件结构组织内容
2. 充分利用提供的所有数据（园区画像、能源倾向、措施列表、政策文档）
3. 语言风格参照国家政策文件，专业严谨
4. 常用表述：绿色低碳循环发展、生态文明建设、产业转型升级、节能降碳、清洁生产、示范引领
5. 明确标注数据来源和推断依据
6. 对数据缺口和不确定性保持透明
7. 提供具体、可操作的建议
8. 适当使用表格、列表等结构化呈现
