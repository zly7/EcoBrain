...
class DataIntakeAgent(BaseAgent):
    ...
    def _profile_csv(self, path: Path, artifacts_dir: Path) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
        ...
        desc_md = self.llm.markdown(
            system_prompt="""你是资深的多能源园区数据分析专家，拥有15年以上工业园区能源管理、碳排放核算和低碳规划经验。你精通：
1. 能源数据质量评估与业务价值挖掘
2. 工业园区能源消耗模式识别与异常检测
3. 碳排放核算边界界定与数据溯源
4. 能源数据与政策条款的关联分析
...
你的分析风格：
- 深度洞察：不仅描述"是什么"，更要分析"为什么"和"意味着什么"
- 全面关联：将数据与行业特征、政策要求、技术路径建立联系
- 风险敏感：主动识别数据质量风险、业务逻辑风险、合规风险
- 价值导向：明确指出数据可支持的决策场景和需要补充的关键信息
- 可审计性：所有判断都基于数据特征，明确标注推断依据和不确定性

请将数据画像转化为深度专业的中文 Markdown 分析报告，为后续的碳排放分析、减排规划和政策匹配提供坚实的数据基础。""",
            user_prompt=prompt,
            fallback=fallback,
        )
        desc = {
    ...
    def _csv_description_prompt(self, profile: Dict[str, Any]) -> str:
        ...
        return (
            "你将获得一份 CSV 数据画像（JSON）。请输出深度专业的 Markdown 分析报告，包含以下章节：\n"
            "\n"
            "## 1. 业务对象识别与场景推断\n"
            "- 基于字段名、数据类型、统计特征，推断这份数据代表的业务对象/过程\n"
            "- 分析可能的数据来源（人工记录/自动采集/第三方报送）\n"
            "- 识别数据所属的管理层级（企业级/园区级/设备级）\n"
            "- 标注推断的置信度和关键假设\n"
            "\n"
            "## 2. 时间维度深度分析\n"
            "- 识别时间字段及其格式\n"
            "- 推断时间粒度（年度/月度/日度/小时级/实时）\n"
            "- 评估时间跨度的充分性（是否足够支持趋势分析、基线建立）\n"
            "- 识别时间序列的连续性风险（缺失月份、异常跳跃）\n"
            "\n"
            "## 3. 关键字段深度解读\n"
            "挑选 8-12 个最重要字段，对每个字段进行：\n"
            "- 业务含义解释（结合行业知识）\n"
            "- 数据类型与取值范围分析\n"
            "- 缺失率及其对分析的影响\n"
            "- 与碳排放核算/能源规划的关联性\n"
            "- 数据质量风险（异常值、逻辑矛盾、单位不明）\n"
            "\n"
            "## 4. 数据质量综合评估\n"
            "- 完整性：缺失严重的字段及其影响范围\n"
            "- 一致性：字段间的逻辑关系检查（如：总量 = 各分项之和）\n"
            "- 准确性：数值范围的合理性（基于行业经验判断）\n"
            "- 时效性：数据更新频率是否满足规划需求\n"
            "- 可追溯性：是否包含数据来源、审核标识等元数据\n"
            "\n"
            "## 5. 政策与标准关联分析\n"
            "- 识别与国家/地方政策要求相关的字段（如：能耗强度、碳排放因子）\n"
            "- 评估数据是否满足政策申报要求（如：绿色工厂、零碳园区）\n"
            "- 指出需要补充的政策合规性数据\n"
            "\n"
            "## 6. 报告章节支撑能力矩阵\n"
            "明确说明这份数据可以支持哪些报告章节，以及支撑强度：\n"
            "- ✅ 强支撑：数据完整、可直接使用\n"
            "- ⚠️ 弱支撑：数据部分缺失或需要补充假设\n"
            "- ❌ 不支撑：关键数据缺失，无法支持该章节\n"
            "\n"
            "涵盖章节：\n"
            "- 园区现状分析（产业结构、能源消耗、碳排放基线）\n"
            "- 能源流分析（供需平衡、损耗分布、峰谷特征）\n"
            "- 经济性分析（投资成本、运营费用、收益测算）\n"
            "- 减排措施评估（技术可行性、减排潜力、经济性）\n"
            "- 政策匹配分析（补贴申请、合规要求、激励政策）\n"
            "- 风险评估（技术风险、经济风险、政策风险）\n"
            "\n"
            "## 7. 关键数据缺口与补充建议\n"
            "- 列出影响分析质量的 Top 5 数据缺口\n"
            "- 对每个缺口说明：缺失内容、影响范围、优先级、获取途径\n"
            "- 提供数据补充的具体建议（调研方法、数据源、替代方案）\n"
            "\n"
            "## 8. 下游分析建议\n"
            "- 基于当前数据质量，建议采用的分析方法（精确计算/经验估算/情景分析）\n"
            "- 提示需要特别关注的不确定性因素\n"
            "- 建议的数据验证和交叉核对方法\n"
            "\n"
            "---\n"
            "**分析要求**：\n"
            "- 语言专业但易懂，避免过度技术化\n"
            "- 所有判断必须基于数据特征，明确标注推断依据\n"
            "- 不要发明具体数值，如无法判断则明确写\"无法从字段判断，建议...\"\n"
            "- 主动识别潜在风险，不回避数据质量问题\n"
            "- 提供可操作的改进建议，而非泛泛而谈\n"
            "\n"
